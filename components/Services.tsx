...
import { Search, Settings2, Trophy, Droplets, Trash2, ShieldAlert, SlidersHorizontal } from 'lucide-react';
  ...
  const sections = [
    {
      ...
      items: [
        ...
        { name: 'Glo Airtime', logo: '/media/glo.jpg', type: 'airtime' },
            ...
            <div className="grid grid-cols-4 gap-y-6">
              {section.items.map((item, i) => (
                <div 
                  ...
                  onClick={() => {
                    if (item.type === 'airtime') onSelectAirtime();
                    else onSelectService(item.name);
                  }}
                >
                  <div className="w-14 h-14 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center justify-center p-3 group-active:scale-95 transition-all">
                    {item.logo ? (
