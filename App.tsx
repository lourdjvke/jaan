...
const App: React.FC = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        if (!user.emailVerified) {
          if (currentScreen !== ScreenState.VERIFICATION_SENT) setCurrentScreen(ScreenState.VERIFICATION_SENT);
          return;
        }
        ...
        onValue(userRef, (snapshot) => {
          ...
          if (isAtGateway) {
             if (!kyc) setCurrentScreen(ScreenState.KYC_FORM);
             else if (!pin) setCurrentScreen(ScreenState.TRANSACTION_PIN);
             else if (lastStep === ScreenState.SUCCESS) setCurrentScreen(ScreenState.SUCCESS);
             else setCurrentScreen(ScreenState.DASHBOARD);
          } else if (currentScreen === ScreenState.DASHBOARD && !pin) {
             setCurrentScreen(ScreenState.TRANSACTION_PIN);
          }
        }, { onlyOnce: true });
      }
    });
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case ScreenState.SIGN_UP: return <SignUp onNext={(email) => { setUserEmail(email); navigateTo(ScreenState.VERIFICATION_SENT); }} onBack={() => navigateTo(ScreenState.ONBOARDING_2, true)} onLogin={() => navigateTo(ScreenState.LOGIN)} onSocialClick={() => showToast("Coming soon")} />;
      case ScreenState.LOGIN: return <Login onBack={() => navigateTo(ScreenState.SIGN_UP, true)} onSignUp={() => navigateTo(ScreenState.SIGN_UP)} onForgot={() => navigateTo(ScreenState.FORGOT_PASSWORD)} onSocialClick={() => showToast("Coming soon")} />;
      case ScreenState.FORGOT_PASSWORD: return <ForgotPassword onBack={() => navigateTo(ScreenState.LOGIN, true)} />;
      ...
      case ScreenState.DASHBOARD: return <Dashboard 
          onLogout={async () => { await auth.signOut(); setCurrentScreen(ScreenState.LOGIN); }} 
